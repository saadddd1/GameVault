...
- **sharp** — WebP 缩略图生成
- **PM2** + **nginx** — 生产部署（1C1G Debian 12 服务器，Cloudflare CDN）
...
| `/api/webhook/github` | POST | HMAC | GitHub push → master 自动部署（`X-Hub-Signature-256` 验证） |
| `/api/health` | GET | 无 | 健康检查：status/uptime/memory |
...
| `lib/cache.ts` | — | 内存缓存：`getCached(key, loader, ttl)` + `invalidate(key)`，60 秒 TTL，每 5 分钟清理过期条目 |
| `lib/lock.ts` | — | `Mutex` 异步互斥锁，保护 DataStore 写操作 |
...
1C1G Debian 12，80Mbps/300GB 流量，Cloudflare 免费 CDN（HKG 边缘节点）。部署文件：

- `ecosystem.config.js` — PM2，单实例 fork 模式，384MB heap，450MB 重启，最多 50 次重启
- `nginx.conf` — 反向代理 + gzip + 静态缓存（`_next/static` 30d，图片 7d）+ 页面微缓存（30s）
- `setup.sh` — 一键部署（安装依赖、构建、启动 PM2、配置 nginx）
- `update.sh` — 从 GitHub 拉取代码 → 构建 → 重启（不覆盖 `data/` 数据文件）
- `backup.sh` — 每小时备份 `data/` 到 `/opt/backups/gemevault/`，保留 30 份
- `deploy.sh` — 本地一键部署：推送 GitHub → 同步数据/图片 → 服务器更新 → 验证
- `harden.sh` — 服务器安全加固（ufw、fail2ban、自动更新、权限）

### Cloudflare 缓存

- 免费 Cache Rules：`(http.request.uri.path wildcard "/*" and http.request.method eq "GET")` 缓存所有 GET 页面
- nginx 已剥离 `Vary` 头（`proxy_hide_header Vary`），否则 Cloudflare 不会缓存
- 静态资源：`/_next/static` 和 `/images` 由 nginx 直接 serve，不走 Next.js

### 重要：nginx API 不缓存

**API 路由禁止缓存。** 曾经在 `/api/` location 配置了 `proxy_cache`（30s），导致 DELETE 操作后 GET 仍返回旧数据（已删除的游戏"复活"）。当前配置为 `Cache-Control: no-cache, no-store, must-revalidate`，确保 API 响应始终实时。
