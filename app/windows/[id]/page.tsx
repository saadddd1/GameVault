...
export default function WindowsDetailPage() {
  ...
  const [app, setApp] = useState<WindowsApp | null>(null)
  ...
  const fetchApp = async () => {
    try {
      ...
      if (data.apps) {
        const found = data.apps.find((a: WindowsApp) => a.id === parseInt(params.id as string))
        setApp(found || null)
      }
    } catch (error) {
  ...
  if (!app) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center">
        ...
        <Link href="/windows" className="px-6 py-3 bg-[#1E3A5F] text-white rounded-sm hover:bg-[#162d47] transition-colors">
          返回Windows软件
        </Link>
      </div>
    )
      ...
      <div className="bg-white border-b border-stone-200">
        <div className="container mx-auto px-4 py-3">
          <nav className="flex items-center gap-2 text-sm text-stone-500">
            ...
            <span>/</span>
            <Link href="/windows" className="hover:text-[#1E3A5F] transition-colors">Windows软件</Link>
            <span>/</span>
                  ...
                  <div className="space-y-3">
                    {app.downloadLinks.map((link, index) => (
                      <div key={index} className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 bg-white rounded-sm border border-stone-100">
                        ...
                        <a
                          href={`/api/download?type=windows&id=${app.id}&index=${index}`}
          ...
          <div className="border-t border-stone-100 px-6 lg:px-8 py-4 bg-stone-50">
            <div className="flex flex-wrap items-center justify-between gap-4 text-sm text-stone-500">
              ...
              <div className="flex items-center gap-4">
                <FeedbackButton targetType="windows" targetId={app.id} targetName={app.name} />
              </div>
            </div>
          </div>
        </div>
