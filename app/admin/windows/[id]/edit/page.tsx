...
export default function EditWindowsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (data.apps) {
          const app = data.apps.find((a: WindowsApp) => a.id === parseInt(params.id as string))
          if (app) {
            setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) { setSuccess('更新成功！'); setTimeout(() => router.push('/admin/windows'), 1500) }
      else { const data = await response.json(); setError(data.error || '更新失败') }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      <h2 className="text-xl font-semibold text-gray-900 mb-6">编辑Windows应用</h2>
